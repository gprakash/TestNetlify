...
describe("createWhereAndParams", () => {
    test("should be a function", () => {
        expect(create_where_and_params_1.default).toBeInstanceOf(Function);
    });
    test("should return the correct clause with 1 param", () => {
        ...
        const varName = "this";
        const node = new test_1.NodeBuilder().instance();
        // @ts-ignore
        const context = { neoSchema: { nodes: [] } };
        const result = create_where_and_params_1.default({ whereInput, varName, node, context });
        expect(result[0]).toEqual(`WHERE this.title = $this_title`);
        expect(result[1]).toMatchObject({ this_title: whereInput.title });
    });
});
//# sourceMappingURL=create-where-and-params.test.js.map