...
function checkNeo4jCompat({ driver, driverConfig }) {
    ...
    `;
        try {
            ...
            const info = result.records[0].toObject();
            const errors = [];
            if (semver_1.default.lt(semver_1.default.coerce(info.version), constants_1.MIN_NEO4J_VERSION)) {
                errors.push(`Expected minimum Neo4j version: '${constants_1.MIN_NEO4J_VERSION}' received: '${info.version}'`);
            }
            if (semver_1.default.lt(semver_1.default.coerce(info.apocVersion), constants_1.MIN_APOC_VERSION)) {
                errors.push(`Expected minimum APOC version: '${constants_1.MIN_APOC_VERSION}' received: '${info.apocVersion}'`);
            }
            const missingFunctions = constants_1.REQUIRED_APOC_FUNCTIONS.filter((f) => !info.functions.includes(f));
            if (missingFunctions.length) {
                errors.push(`Missing APOC functions: [ ${missingFunctions.join(", ")} ]`);
            }
            const missingProcedures = constants_1.REQUIRED_APOC_PROCEDURES.filter((p) => !info.procedures.includes(p));
