...
describe("cursor-pagination", () => {
    describe("createOffsetLimitStr", () => {
        test("it returns an empty string when no offset or limit is provided", () => {
            const args = {};
            const result = pagination_1.createOffsetLimitStr(args);
            expect(result).toBe("");
        });
        ...
        test("it returns a string with the full range of the slice when Integers are passed for both ski and limit", () => {
            const args = { limit: neo4j_driver_1.default.int(10), offset: neo4j_driver_1.default.int(30) };
            const result = pagination_1.createOffsetLimitStr(args);
            expect(result).toBe("[30..40]");
            const mixedArgs = { limit: neo4j_driver_1.default.int(30), offset: 20 };
            const result2 = pagination_1.createOffsetLimitStr(mixedArgs);
    ...
    describe("createConnectionWithEdgeProperties", () => {
        test("it should throw an error if first is less than 0", () => {
            ...
            const totalCount = 50;
            expect(() => {
                pagination_1.createConnectionWithEdgeProperties({
        ...
        test("it returns all elements if the cursors are invalid", () => {
            ...
            const totalCount = 50;
            const result = pagination_1.createConnectionWithEdgeProperties({
                source: { edges: arraySlice },
                args,
                totalCount,
                selectionSet: undefined,
            ...
            expect(result).toStrictEqual({
                edges: arraySlice.map((edge, index) => (Object.assign(Object.assign({}, edge), { cursor: graphql_relay_1.offsetToCursor(index) }))),
                pageInfo: {
                    hasNextPage: false,
                    hasPreviousPage: false,
                    startCursor: graphql_relay_1.offsetToCursor(0),
                    endCursor: graphql_relay_1.offsetToCursor(19),
                },
            });
        });
        ...
        test("it should return cursors from 11 to 31 when the after argument is provided and the array size is 20", () => {
            const arraySlice = [...Array(20).keys()].map((key) => ({ node: { id: key } }));
            const args = { after: graphql_relay_1.offsetToCursor(10) };
            const totalCount = 50;
            const result = pagination_1.createConnectionWithEdgeProperties({
                source: { edges: arraySlice },
                args,
            ...
            expect(result).toStrictEqual({
                edges: arraySlice.map((edge, index) => (Object.assign(Object.assign({}, edge), { cursor: graphql_relay_1.offsetToCursor(index + 11) }))),
                pageInfo: {
                    hasNextPage: false,
                    hasPreviousPage: true,
                    startCursor: graphql_relay_1.offsetToCursor(11),
                    endCursor: graphql_relay_1.offsetToCursor(30),
                },
            });
        });
    });
